<template>
    <section class="py-20 bg-gray-100 overflow-x-hidden">
        <div class="container px-4 mx-auto">
            <div class="relative max-w-4xl mx-auto">
                <div class="absolute inset-0 bg-blue-200 my-12 -ml-12 -mr-12"></div>
                <div class="relative pt-20 pb-32 px-8 bg-blue-300 text-center">
                    <a class="inline-block mb-14" href="#">
                        <img class="h-9" src="https://shuffle.dev/yofte-assets/logos/yofte-logo-white.svg" alt="">
                    </a>
                    <h2 class="mb-5 text-4xl md:text-5xl text-white font-bold font-heading">Join Our Newsletter</h2>
                    <p class="mb-6 text-blue-100">Sign me up for Yofte newsletters to get exclusive perks.</p>
                    <img class="mb-12 inline-block" src="https://shuffle.dev/yofte-assets/elements/quote.svg" alt="">
                    <div class="relative max-w-xl mx-auto bg-white rounded-lg">
                        <img class="absolute top-0 right-0 h-40 -mt-12 -mr-20" src="https://shuffle.dev/yofte-assets/elements/quote-light.png"
                            alt="">
                        <div class="relative lg:max-w-xl lg:mx-auto bg-white rounded-lg">
                            <div class="relative flex flex-wrap items-center justify-between">
                                <div class="relative flex-1">
                                    <span
                                        class="absolute top-0 left-0 ml-8 mt-4 font-semibold font-heading text-xs text-gray-400">Drop
                                        your e-mail</span>
                                    <input
                                        class="inline-block w-full pt-8 pb-4 px-8 placeholder-gray-900 border-0 focus:ring-transparent focus:outline-none rounded-md"
                                        type="text" placeholder="steven@example.dev">
                                </div>
                                <a class="inline-block w-auto bg-blue-800 hover:bg-blue-900 text-white font-bold font-heading py-6 px-8 rounded-md uppercase text-center"
                                    href="#">Join</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>